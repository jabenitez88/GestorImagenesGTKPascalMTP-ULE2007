...
Const
	...
	IDIMG = 13;					{ En nuestra tabla de tipo tTablaBusqueda, la ID es el campo 11, por eso vale 12. 	}
	UNICOELEM = 0;					{ Esto nos sirve para, al modificar una imagen, coger sólo la info de un elemento, el  	
  	...
  	ancho = 1000;					{ Anchura de la ventana principal del programa.						}
...
Var
	...
	rbutton_fecha		: PGtkWidget;		{ Buscar por fecha	 }
...
Begin
  ...
  if (criterio=(StrPas(bEti))) then 
			...
			End
  else if (criterio=bTem) then begin				{ Para sacar todas las imágenes que contienen una temática, ya sea por ser ella misma, hija de ella, nieta...  }
  ...
  else if(criterio=bFecha) then
			begin
			busqueda := busquedaIMG2a + BusquedaIMG2b + criterio + texto + '"';
			//writeln(busqueda);
			BusquedaDB(@busqueda[1],elemtotals,true,tabaux);
			end;
  tabla := tabaux;
...
Const
  	titles : ARRAY[0..11] OF pgchar = ('Nombre','Temática', 'Tem-Padre', 'Tipo', 'Filesize', 'Ancho', 'Alto', 'Etiquetas','Descripcion', 'Imagen','Identificador','Fecha');
        num_menu_items = 11;
...
Begin
	 ...
	 if (tipoboton=2) then Begin
         	...
         	rbutton_fecha := gtk_radio_button_new_with_label(gtk_radio_button_get_group(GTK_RADIO_BUTTON (rbutton_tem)),'Fecha');
         	gtk_container_add(GTK_CONTAINER(bbox), rbutton_fecha);
...
Begin	imagen.solouna := TRUE;
	...
	gtk_container_add(pGtkContainer(frame_listado), scroll);
        ...
        thelist := gtk_clist_new_with_titles(12, titles);
	...
	{ Es necesario indicar el ancho de las columnas para que no se solapen. La primera columna es la 0 }
        ...
        gtk_clist_set_column_width(pGtkCList(thelist), 10, 80);
        gtk_clist_set_column_width(pGtkCList(thelist), 11, 25);
	...
	gtk_signal_connect_object(pGtkObject(rbutton_fecha), 'clicked', tGtksignalfunc(@asignar_criterio), bFecha);
